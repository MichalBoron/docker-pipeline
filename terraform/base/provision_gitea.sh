...
# arg1: number of retries
# arg2: interval between retries in seconds
function gitea_probe() {
  for i in $(seq 1 $1)
  do
...
# max 10 requests, 3 second intervals
gitea_probe 10 3
if [[ $? == 1 ]]
then
...
# max 15 requests, 2 second intervals
gitea_probe 15 2
if [[ $? == 1 ]]
then
  >&2 echo "Gitea server is not present at http://localhost:3000/"
